...
static struct redact_task *read_policy(void) {
	const uint8_t buf[MAX_MESSAGE_SIZE];
	int real_size;
	int fd = open(opts.policy_path, O_RDONLY);
	int i, j;
...
int page_xfer_dump_pages(struct page_xfer *xfer, struct page_pipe *pp)
{
	...
	struct redact_task *redact_tasks = NULL;
	if (opts.policy_path) redact_tasks = read_policy();
